...
> This is a fork of [khoj-ai/openpaper](https://github.com/khoj-ai/openpaper) with modifications for self-hosting with any OpenAI-compatible API endpoint and Cloudflare R2 storage.
...
All LLM calls — both server-side chat/assistant features and the background jobs
service for PDF metadata extraction — now use your own OpenAI-compatible endpoint.
No Google Gemini or Anthropic API keys required. Just set these env vars:
...
The server and jobs service share the same endpoint configuration.

### Cloudflare R2 storage
PDFs, preview images, extracted figures, and audio overviews are stored on
Cloudflare R2 (or any S3-compatible storage). Set the `S3_ENDPOINT_URL`
environment variable to point at your provider:
...
CLOUDFLARE_BUCKET_NAME=your-public-domain.r2.dev
```

When `S3_ENDPOINT_URL` is not set, the stack falls back to standard AWS S3.

### Dev mode auth bypass
When `DEBUG=True` (in `server/.env`), all auth is bypassed — you're automatically
logged in as a local `dev@localhost.com` user that's created on first request.
No Google OAuth setup needed for local development.
