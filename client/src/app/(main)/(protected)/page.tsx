...
const DEFAULT_PAPER_UPLOAD_ERROR_MESSAGE =
...
export default function Home() {
    ...
    const [jobUploadStatus, setJobUploadStatus] = useState<JobStatus | null>(
        null,
    );
    ...
    const [errorAlertMessage, setErrorAlertMessage] = useState(
        DEFAULT_PAPER_UPLOAD_ERROR_MESSAGE,
    );
    ...
    const isUploadBlocked =
        !subscriptionLoading &&
        (isPaperUploadAtLimit(subscription) || isStorageAtLimit(subscription));

    // Handler to show error when upload is blocked
    const handleUploadBlocked = () => {
        if (isPaperUploadAtLimit(subscription)) {
            setErrorAlertMessage(
                "You've reached your paper upload limit. Please upgrade your plan to upload more papers.",
            );
        } else if (isStorageAtLimit(subscription)) {
            setErrorAlertMessage(
                "You've reached your storage limit. Please upgrade your plan or delete some papers to continue.",
            );
        }
        ...
        if (!subscriptionLoading && subscription && user) {
            ...
            if (isStorageAtLimit(subscription)) {
                toast.error("Storage limit reached", {
                    description:
                        "You've used your available storage. Upgrade for more space, or free up room by removing papers.",
                    action: {
                        label: "Upgrade",
                ...
                toast.error("Upload limit reached", {
                    description:
                        "You've used your available paper uploads. Upgrade for more, or remove existing papers.",
                    action: {
                        label: "Upgrade",
                        onClick: () => (window.location.href = "/pricing"),
                    },
                ...
                toast.warning("Upload limit approaching", {
                    description:
                        "You're getting close to your paper upload limit.",
                    action: {
                        label: "Upgrade",
                        onClick: () => (window.location.href = "/pricing"),
                    },
    ...
    useEffect(() => {
        ...
        const typingTimer = setInterval(() => {
            const currentMessage =
                celeryMessage || loadingMessages[messageIndex];
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const papersResponse = await fetchFromApi(
                    "/api/paper/relevant",
                );
                        ...
                        <div className="flex justify-end mt-4">
                            {showPricingOnError && (
                                <Button
                                    variant="default"
                                    asChild
                                    className="mr-2 bg-blue-500 hover:bg-blue-200 dark:bg-blue-600 dark:hover:bg-blue-700 text-white"
                                >
