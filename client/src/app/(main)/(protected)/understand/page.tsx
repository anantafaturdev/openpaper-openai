...
function UnderstandPageContent() {
        ...
        async (e: FormEvent | null = null) => {
            ...
            if (!currentConversationId) {
                try {
                    ...
                    window.history.pushState(
                        null,
                        "",
                        `/understand?id=${currentConversationId}`,
                    );
                } catch (error) {
            ...
            if (!mentionSelectionIsEmpty(submittedMentions)) {
                if (submittedMentions.paperIds.length > 0) {
                    requestBody.mentioned_paper_ids =
                        submittedMentions.paperIds;
                }
                if (submittedMentions.projectIds.length > 0) {
                    requestBody.mentioned_project_ids =
                        submittedMentions.projectIds;
                }
                if (submittedMentions.highlights.length > 0) {
                    requestBody.mentioned_highlight_ids =
                        submittedMentions.highlights.map((h) => h.id);
                }
            }
                        ...
                        try {
                            ...
                            if (
                                ...
                                if (chunkType === "content") {
                                    ...
                                    // The upstream model connection dropped mid-answer
                                    // and the server re-sent the request. The partial
                                    // answer is not resumable, so clear it before the
                                    // replacement answer streams in. Artifacts and the
                                    // trace come from the earlier evidence phase and
                                    // survive the restart.
                                    console.warn(
    ...
    const refreshPaperUrl = useCallback(
        async (paperId: string): Promise<string | null> => {
            try {
