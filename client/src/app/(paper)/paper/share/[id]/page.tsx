...
export default function SharedPaperView() {
    ...
    useEffect(() => {
        ...
        const fetchSharedData = async () => {
            ...
            setError(null);
            try {
                const response: SharedPaper = await fetchFromApi(
                    `/api/paper/share?id=${shareId}`,
                );
                ...
                setError(
                    "Failed to load shared paper. The link might be invalid or expired.",
                );
                setPaperData(null);
                setHighlights([]);
                setAnnotations([]);
        ...
        const fetchConversation = async () => {
            try {
                const response = await fetchFromApi(
                    `/api/conversation/share/${shareId}`,
                );
                        ...
                        <div className="w-full h-full flex flex-row relative pr-[60px]">
                            <div className="flex-grow overflow-y-auto">
                                ...
                                {rightSideFunction === "Overview" && (
                                    <div
                                        className={
                                            "flex flex-col md:px-2 m-2 relative animate-fade-in"
                                        }
                                    >
                            ...
                            paperData.summary && (
                                ...
                                >
                                    ...
                                    <div className="prose dark:prose-invert !max-w-full text-sm">
                                        {memoizedOverviewContent}
                                        {paperData.summary_citations &&
                                            paperData.summary_citations.length >
                                                0 && (
                                                <div
                                                    className="mt-0 pt-0 border-t border-gray-300 dark:border-gray-700"
                                                    id="references-section"
