...
function getAnnotationCardAnchorFromOverlay(
    ...
    if (pageEl) {
        ...
        const pageRightContent =
            pageRect.right - containerRect.left + scrollLeft;
        const pageLeftContent = pageRect.left - containerRect.left + scrollLeft;
        const rightGutterLeft =
            pageRightContent + ANNOTATION_CARD_MARGIN_GAP_PX;
        const leftGutterLeft =
            pageLeftContent -
            ANNOTATION_CARD_MARGIN_GAP_PX -
            ANNOTATION_CARD_WIDTH_PX;
        let left = rightGutterLeft;
        if (
            rightGutterLeft + ANNOTATION_CARD_WIDTH_PX >
...
function getAnnotationCardAnchorForHighlight(
    ...
    if (!highlight.position)
        return getAnnotationCardAnchorFromOverlay(
            highlight.id,
            scrollContainer,
        );
    ...
    const usePdfCoords = !!(
        highlight.position as { usePdfCoordinates?: boolean }
    ).usePdfCoordinates;
    ...
    const top =
        pageRect.top - containerRect.top + scrollContainer.scrollTop + yOffset;
...
export function PdfHighlighterViewer(props: PdfHighlighterViewerProps) {
    ...
    const [selectionRectTop, setSelectionRectTop] = useState<number | null>(
        null,
    );
    ...
    const [pendingSidePanelAnnotate, setPendingSidePanelAnnotate] =
        useState(false);
    /** When inline cards are hidden, user clicked a highlight with a persisted thread — show that card only */
    const [peekAnnotationCardHighlightId, setPeekAnnotationCardHighlightId] =
        useState<string | null>(null);
    /** Hover card: show annotations on highlight hover when margin cards are hidden */
    const [hoverHighlightId, setHoverHighlightId] = useState<string | null>(
        null,
    );
    ...
    const [currentSelection, setCurrentSelection] =
        useState<PdfSelection | null>(null);
    ...
    const togglePdfDarkMode = useCallback(() => {
        setPdfDarkMode((prev) => {
            const next = !prev;
            localStorage.setItem("pdfDarkMode", String(next));
            return next;
        });
    }, []);
    ...
    const layoutTickDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(
        null,
    );
     ...
     */
    const [pdfHighlightsSuppressedForZoom, setPdfHighlightsSuppressedForZoom] =
        useState(false);
    const pendingZoomHighlightRef = useRef(false);
    const zoomHighlightDebounceRef = useRef<ReturnType<
        typeof setTimeout
    > | null>(null);
        ...
        const unsuppressAfterPaint = () => {
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    requestAnimationFrame(() => {
                        if (
                            session !== zoomHighlightSessionRef.current ||
                            !pendingZoomHighlightRef.current
                        )
                            return;
        ...
        Object.defineProperty(viewer as object, "currentScaleValue", {
            ...
            set(value: string) {
                ...
                if (pendingZoomHighlightRef.current) {
                    ...
                    if (
                        !isNaN(num) &&
                        Math.abs(num - scaleRef.current) > 0.001
                    ) {
                        return;
                    }
                }
    ...
    const syncAnnotationCardPositions = useCallback(() => {
        setAnnotationCards((prev) => {
            ...
            const viewer = highlighterUtilsRef.current?.getViewer();
            const scrollContainer = viewer?.container as
    ...
    const highlightHasPersistedAnnotations = useCallback(
        (highlightId: string | undefined) =>
            Boolean(
                highlightId &&
                annotations.some((a) => a.highlight_id === highlightId),
            ),
        [annotations],
    );
    ...
    const ensureAnnotationCardEntry = useCallback(
        (highlightId: string) => {
            setAnnotationCards((prev) => {
                if (prev.some((c) => c.highlightId === highlightId))
                    return prev;
                ...
                const firstAnn = annotations.find(
                    (a) => a.highlight_id === highlightId,
                );
                return [
                    ...prev,
                    {
        ...
        (selection: PdfSelection) => {
            ...
            if (anchorBottom === null) {
                ...
                const pageView = viewer?.getPageView(
                    boundingRect.pageNumber - 1,
                );
                if (pageView?.div && pageView?.viewport) {
                    const pageRect = (
                        pageView.div as HTMLElement
                    ).getBoundingClientRect();
                    const scaleX = pageView.viewport.width / boundingRect.width;
                    const scaleY =
                        pageView.viewport.height / boundingRect.height;
    ...
    const handleHighlightClick = useCallback(
        (
            ...
            setSelectedText(
                viewportHighlight.content?.text ||
                    viewportHighlight.raw_text ||
                    "",
            );
            setSelectionRectTop(event.clientY);

            const hid = viewportHighlight.id;
            const originalHighlight = hid
                ? extendedHighlights.find((h) => h.id === hid)
                : undefined;
            ...
            const openMenuAtHighlight = () => {
                ...
                const partsContainer = target.classList.contains(
                ...
                if (partsContainer) {
                    const parts = Array.from(
                        partsContainer.querySelectorAll(".TextHighlight__part"),
                    );
                    if (parts.length > 0) {
            ...
            if (showAnnotationCards) {
                if (!card) {
                    ...
                    const targetScrollTop =
                        card.top - container.clientHeight / 2;
                    container.scrollTo({
                ...
                if (ph?.position) {
                    ...
                    if (viewer && scrollContainer) {
                        ...
                        if (anchor) {
                            ...
                            const targetScrollTop =
                                anchor.top - scrollContainer.clientHeight / 2;
    ...
    useEffect(() => {
        ...
        const useSidePanel =
            !showAnnotationCards &&
            annotationsPanelActive &&
            Boolean(onAnnotateViaSidePanel);
    ...
    const annotationIdsKey = annotations
        .map((a) => a.id)
        .sort()
        .join(",");
        ...
        setAnnotationCards((prev) =>
            prev.filter(
                (card) =>
                    !card.annotationId ||
                    annotations.some(
                        (a) => a.highlight_id === card.highlightId,
                    ),
            ),
        );
    }, [annotations]);
    ...
    useEffect(() => {
        ...
        const getHighlightId = (target: EventTarget | null): string | null => {
            ...
            const overlay = el.closest<HTMLElement>(
            ...
            const positioned = el.closest<HTMLElement>(
                "[data-pdf-text-highlight][data-highlight-id]",
            );
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            ...
            {
                root:
                    pdfViewer.closest(".pdfViewerContainer") ||
        ...
        const createOverlaysForTextLayer = (
            ...
            for (const highlight of highlightsWithoutPosition) {
                ...
                const overlayIsActive =
                    ...
                    highlight.id === activeHighlightIdRef.current;
                ...
                overlays.forEach((el) => {
                    ...
                    el.setAttribute(
                        "data-role",
                        highlight.role === "assistant" ? "assistant" : "user",
                    );
                    if (highlight.color)
                        el.setAttribute("data-color", highlight.color);
                    ...
                    el.addEventListener("click", (e) => {
                        ...
                        if (
                            ...
                            if (viewer && scrollContainer) {
                                const anchor =
                                    getAnnotationCardAnchorForHighlight(
                                        highlight,
                                        viewer,
                                        scrollContainer,
                                        numPages,
        ...
        overlays.forEach((el) => {
            ...
            el.style.backgroundColor =
                el.getAttribute("data-role") === "assistant"
                    ? getAssistantHighlightBackgroundRgba(isActive, pdfDarkMode)
                    : getUserHighlightBackgroundRgba(
                          (el.getAttribute(
                              "data-color",
                          ) as HighlightColor | null) ?? undefined,
                          isActive,
                          pdfDarkMode,
                      );
        });
    }, [activeHighlight, pdfDarkMode]);
    ...
    return (
        ...
        >
                            ...
                            <>
                                ...
                                <PdfHighlighter
                                    ...
                                    onCreateGhostHighlight={
                                        handleCreateGhostHighlight
                                    }
                                    onRemoveGhostHighlight={
                                        handleRemoveGhostHighlight
                                    }
                    ...
                    onAnnotate={(y) => {
                        ...
                        if (isHighlightInteraction && activeHighlight?.id) {
                            const h = highlights.find(
                                (x) => x.id === activeHighlight.id,
                            );
                            const anchor = h?.position
                                ? getAnnotationCardAnchorForHighlight(
                                      h,
                                      viewer,
                                      scrollContainer,
                            ...
                            if (anchor) {
                                ...
                                const pdfPage =
                                    ...
                                    document.querySelector(
                                        "#pdf-container [data-page-number]",
                                    );
                                const pageRect =
            ...
            {(showAnnotationCards || peekAnnotationCardHighlightId !== null) &&
                (() => {
                    ...
                    const cardsForLayout = showAnnotationCards
                        ? annotationCards
                        : annotationCards.filter(
                              (c) => c.highlightId === peekId,
                          );
                    const sorted = [...cardsForLayout].sort(
                        (a, b) => a.top - b.top,
                    );
                    const activeIdx = sorted.findIndex(
                        (c) => c.highlightId === activeId,
                    );
                    ...
                    if (activeIdx === -1) {
                        ...
                        adjusted = sorted.map((card) => {
                            const h =
                                cardHeights.get(card.highlightId) ??
                                FALLBACK_HEIGHT;
                        ...
                        for (let i = activeIdx - 1; i >= 0; i--) {
                            const h =
                                cardHeights.get(sorted[i].highlightId) ??
                                FALLBACK_HEIGHT;
                            const top = Math.min(
                                sorted[i].top,
                                Math.max(0, nextCardTop - GAP - h),
                            );
                        ...
                        const activeH =
                            cardHeights.get(sorted[activeIdx].highlightId) ??
                            FALLBACK_HEIGHT;
                        ...
                        createPortal(
                            <InlineAnnotationCard
                                ...
                                onAnnotationSaved={(_savedId) => {
                                    setAnnotationCards((prev) =>
                                        prev.map((c) =>
                                            c.highlightId === card.highlightId
                                                ? {
                                                      ...c,
                                                      annotationId: _savedId,
                                                  }
                                                : c,
                                        ),
                                    );
                                ...
                                onClose={() => {
                                    ...
                                    const hasAnn = annotations.some(
                                        (a) =>
                                            a.highlight_id === card.highlightId,
                                    );
            ...
            {!showAnnotationCards && hoverHighlightId && hoverPosition && (
                <AnnotationHoverCard
                    annotations={annotations.filter(
                        (a) => a.highlight_id === hoverHighlightId,
                    )}
