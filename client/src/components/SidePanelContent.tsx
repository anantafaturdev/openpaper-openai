    ...
    const fetchMoreMessages = useCallback(async () => {
        ...
        try {
            const response = await fetchFromApi(
                `/api/conversation/${conversationId}?page=${pageNumberConversationHistory}`,
                {
                    method: "GET",
                },
            );
            ...
            try {
                ...
                try {
                    if (!retrievedConversationId) {
                        ...
                        const newConversationResponse = await fetchFromApi(
    ...
    const handleSubmit = useCallback(
        async (e: FormEvent | null = null) => {
            ...
            try {
                    ...
                    for (const event of parts) {
                        ...
                        try {
                            ...
                            if (chunkType === "content") {
                                contentChunks++;
                                console.log(
                                    `Processing content chunk #${contentChunks}:`,
                                    chunkContent,
                                );
                                ...
                                referenceChunks++;
                                console.log(
                                    `Processing references chunk #${referenceChunks}:`,
                                    chunkContent,
                                );
                ...
                console.log(
                    `Stream completed. Processed ${chunkCount} chunks (${contentChunks} content, ${referenceChunks} references).`,
                );
                ...
                components={{
                    ...
                    p: (props) => (
                        <CustomCitationLink
                            ...
                            citations={
                                paperData.summary_citations?.map(
                                    (citation) => ({
        ...
        return messages.map((msg, index) => (
            ...
            >
                ...
                <div
                    ...
                    className={`relative group prose dark:prose-invert p-2 !max-w-full rounded-lg ${
                        msg.role === "user"
            ...
            {rightSideFunction !== "Read" && (
                <div className="flex-grow h-full overflow-hidden pr-[60px]">
                    {rightSideFunction === "Annotations" && user && (
                        <div
                            className={`flex flex-col ${heightClass} overflow-y-auto`}
                        >
                            <AnnotationsView
                    ...
                    {rightSideFunction === "Share" && paperData && (
                        <div
                            className={`flex flex-col ${heightClass} p-4 space-y-4`}
                        >
                            ...
                            {paperData.is_public ? (
                                <div className="space-y-3">
                                    <p className="text-sm text-muted-foreground">
                                        This paper is currently public. Anyone
                                ...
                                <div className="space-y-3">
                                    <p className="text-sm text-muted-foreground">
                                        Make this paper public to share it with
                                        others via a unique link. All of your{" "}
                                        <b>annotations and chats</b> will be
                    ...
                    {rightSideFunction === "Audio" && (
                        <div
                            className={`flex flex-col ${heightClass} md:px-2 overflow-y-auto`}
                        >
                            <AudioOverviewPanel
                                paper_id={id}
                            ...
                            >
                                {userMessageReferences.length > 0 && (
                                    <div className="flex flex-row gap-2">
                                        {userMessageReferences.map(
                                            (ref, index) => (
                                                <div
                                                    key={index}
                                                    className="text-xs text-secondary-foreground flex bg-secondary p-2 rounded-lg"
                                                >
                                                    <p
                                                        className="
                                                                    overflow-hidden
                                ...
                                <div className="rounded-md p-0.5 flex flex-col gap-2 bg-secondary">
                                ...
                                {creditUsage && creditUsage.showWarning && (
                                    <div
                                        className={`text-xs px-2 py-1 ${creditUsage.isCritical ? "text-red-600 dark:text-red-400" : "text-amber-600 dark:text-amber-400"} justify-between flex`}
                                    >
