...
_DEFAULT_LLM_PROVIDER = os.getenv("DEFAULT_LLM_PROVIDER", "GEMINI")
...
class BaseLLMClient:
    ...
    def __init__(self, default_provider: Optional[LLMProvider] = None):
        # DEFAULT_LLM_PROVIDER env var is case-insensitive
        env_provider = os.getenv("DEFAULT_LLM_PROVIDER")
        if env_provider:
            try:
                self.default_provider = LLMProvider(env_provider.lower())
            except ValueError:
                self.default_provider = default_provider or LLMProvider.GEMINI
        else:
            self.default_provider = default_provider or LLMProvider.GEMINI
        ...
        if provider not in self._providers:
            if provider == LLMProvider.GEMINI:
                ...
                self._providers[provider] = OpenAIProvider(
                    ...
                    supports_pdf_input=(
                        os.getenv("OPENAI_SUPPORTS_PDF", "true").lower() == "true"
                    ),
                )
